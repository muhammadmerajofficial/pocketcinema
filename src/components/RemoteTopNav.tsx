...
export const RemoteTopNav: React.FC<RemoteTopNavProps> = ({
  ...
  return (
    <div 
      id="remote-top-nav" 
      className="w-full flex items-center justify-between gap-1.5 sm:gap-2 p-1 sm:p-1.5 bg-zinc-900/95 backdrop-blur-md rounded-xl sm:rounded-2xl border border-zinc-800/80 shadow-2xl shadow-black/60"
    >
      {categories.map((cat) => {
        const Icon = cat.icon;
        ...
        return (
          <button
            ...
            className={`relative flex-1 flex items-center justify-center gap-1 sm:gap-2 py-2 sm:py-2.5 px-1.5 sm:px-3 rounded-lg sm:rounded-xl font-medium transition-all duration-300 select-none cursor-pointer focus:outline-none focus:ring-1 sm:focus:ring-2 focus:ring-amber-500/50 ${
              isActive
